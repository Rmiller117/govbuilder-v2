      ...
      <div class="max-w-4xl mx-auto px-6 py-5 flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <h1 class="text-3xl font-bold">
            {{ isNew ? 'Create License Status' : 'Edit License Status' }}
          </h1>
        </div>
      ...
      >
        ...
        <div>
          <label class="block text-sm font-semibold mb-3">
            License Status Title <span class="text-danger">*</span>
          </label>
          <input
            v-model="form.title"
            required
        ...
        <div class="flex justify-end gap-4 pt-8 border-t border-base">
          ...
          >
            {{ isNew ? 'Create' : 'Save' }} License Status
          </button>
        </div>
      </form>
    </main>
...
const form = ref<LicenseStatusForm>({
  title: '',
...
onMounted(() => {
  if (!isNew.value) {
    const existing = licenseStatusStore.list.value.find((s: { id: string }) => s.id === id.value)
    if (!existing) {
      router.replace('/license-statuses')
      return
    }
...
async function save() {
  ...
  if (isNew.value) await licenseStatusStore.add(payload)
  else await licenseStatusStore.update(id.value, payload)

  router.push('/license-statuses')
}
